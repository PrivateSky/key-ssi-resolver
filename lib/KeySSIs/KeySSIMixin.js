...
function keySSIMixin(target) {
    ...
    target.isCompatibleWith = (keySSI) => {
        const iesConfig1 = cryptoRegistry.getCryptoFunction(target, CryptoFunctionTypes.GET_IES_CONFIG)();
        const iesConfig2 = cryptoRegistry.getCryptoFunction(keySSI, CryptoFunctionTypes.GET_IES_CONFIG)();
        for (let prop in iesConfig1) {
            if (iesConfig2.hasOwnProperty(prop)) {
                if (iesConfig1[prop] !== iesConfig2[prop]) {
                    return false;
                }
            } else {
                return false;
