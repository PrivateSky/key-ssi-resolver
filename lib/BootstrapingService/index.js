...
function BootstrapingService(options) {
    ...
    this.versions = (keySSI, callback) => {
        const requestsChain = createRequestsChain('getAllVersions', ANCHOR_SERVICE, keySSI);
        requestsChain.execute(callback);

    }

    this.addVersion = (keySSI, hashLinkSSI, lastHashLinkSSI, callback) => {
        const requestsChain = createRequestsChain('appendToAnchor', ANCHOR_SERVICE, keySSI, hashLinkSSI, lastHashLinkSSI);
        requestsChain.execute(callback);
    }
}
